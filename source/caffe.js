...
caffe.Node = class {
    ...
    constructor(metadata, layer, version, value) {
        ...
        let initializers = [];
        const createAttribute = (metadata, name, value, defaultValue) => {
            let visible = true;
            ...
            if (type) {
                value = caffe.Utility.enum(type, value);
            }
                ...
                for (const layer_kind of Object.keys(layer)) {
                    if (layer_kind.endsWith('_param') || layer_kind === 'transform_param') {
                        ...
                        for (const name of Object.keys(param)) {
                            ...
                            const attribute = createAttribute(metadata.attribute(type, name), name, value, defaultValue);
                            this.attributes.push(attribute);
                        }
                    }
                }
                if (layer.include && layer.include.length > 0) {
                    const attribute = createAttribute(metadata.attribute(type, 'include'), 'include', layer.include);
                    this.attributes.push(attribute);
                }
                if (layer.exclude && layer.exclude.length > 0) {
                    const attribute = createAttribute(metadata.attribute(type, 'exclude'), 'exclude', layer.exclude);
                    this.attributes.push(attribute);
                }
                if (this.type === 'Data' && layer.input_param && layer.input_param.shape) {
                    const attribute = createAttribute(metadata.attribute(type, 'shape'), 'shape', layer.input_param.shape);
                    this.attributes.push(attribute);
                }
